...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', dirname(__FILE__).'/shared/cache');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security) looper 2015-08-07
	...
	public function __construct(){
		...
		if($this->isURL){
			if(ALLOW_ALL_EXTERNAL_SITES){
				...
				// looper bugs fixing for allow sites 2015-08-07
				// --------------------------------------------------------------------------------------
				$allowed = true;
				/*
		...
		if($this->isURL){
			...
			// looper bugs fixing for apache2.alias: files, and ?v={$versionNuM} 2015-08-07
			// --------------------------------------------------------------------------------------
			$this->src = str_replace("/files", dirname(__FILE__)."/shared/sites", $this->src);
			$src = str_replace("/files", dirname(__FILE__)."/shared/sites", $this->src);
            $a = explode('?',$src); if(count($a)>1){$src=$a[0];}
            $this->localImage = $this->getLocalImagePath($src);
            // --------------------------------------------------------------------------------------
			if(! $this->localImage){
